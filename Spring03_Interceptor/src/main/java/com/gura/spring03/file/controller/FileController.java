...
public class FileController {
	...
	public String upload(FileDto dto,HttpServletRequest request) {//FileDto 의 필드에 MultpartFile 객체의 참조값이 들어 있다.
	...
	public String upload(String title, MultipartFile myFile,HttpServletRequest request) {
		/**
		 * 입력한 제목은 title 에 들어있고 , 업로드된 파일에 대한 정보는 myFile 객체에 들어있다.
		 * 따라서 myFile 객체의 메소드를 활용해서 업로드 처리에 필요한 정보를 얻어내고 어떤 동작을 수행하면된다.
		 * 팜고로 일반적인 웹서버 application 은  클라이언트가 파일 업로드를 해오면 운영체제의 임시폴더에
		 * 이상한(긴) 파일명으로 일단 저장을 해놓는다.
		 * 일정시간 이후에 임시 폴더에 저장되었던 파일은  운영체제가 자동으로 지우거나 비운다.
		 * 따라서 해당 파일을 사용하기 위해서는 원하는 위치로 이동(copy) 시켜야한다.
		 */
	      //1. 원본 파일명
	      String orgFileName=myFile.getOriginalFilename();
	      //2. 파일의 크기
