...
<body>
   ...
   <script>	
   		...
   		document.querySelector("#submitBtn").addEventListener("click",()=>{
   			...
   			const files=ocument.querySelector("#image").files;
   			//만일 caption 을 3글자 미만으로 입력하거나 선택한 파일이 없다면
   			if(caption.length < 3 || files.length==0){
   				alert("caption 을 3글자 이상 입력하고 업로드할 이미지를 선택해 주세요");
   				return; //함수를 여기서 종료
   			}
			...
			/*
				...
				<form method="post" enctype="multpart/form-data"> 폼을 전송한것과 같다.
			*/
   		...
   		//form 이 밖으로 나와있어서 강제로 서브밋해버리는 코드
   		document.querySelector("#submitBtn").addEventListener("click",()=>{
			//버튼submitBtn을 강제로 제출시키기
			document.querySelector("form").submit();
		});
   	
   		//dropZone 을 클릭해도 파일 선택창을 띄우도록한다.
   		document.querySelector("#dropZoneLink").addEventListener("click",()=>{
   			//input type-"file" 을 강젲 클릭시킨다.
